      ...
      <div className="max-w-5xl mx-auto space-y-8 animate-fade-in">
        ...
        <div className="bg-white/10 p-8 rounded-3xl border border-white/10 shadow-2xl backdrop-blur-md">
          <div className="flex justify-between items-end mb-4">
            <div>
              <h3 className="text-2xl font-black text-white">
                {targetLanguage === 'eu' ? 'Aurrerapen Nagusia' : 'Progreso General'}
              </h3>
              <p className="text-gray-400 text-sm">
                {targetLanguage === 'eu' ? 'Esaldi guztien ikaskuntza-mailan oinarritua' : 'Basado en el nivel de aprendizaje de todas las frases'}
              </p>
            </div>
          ...
          <div className="grid grid-cols-3 gap-2 mt-4 text-[10px] uppercase tracking-widest font-bold text-center">
            <div className="text-gray-500">{targetLanguage === 'eu' ? 'Berriak' : 'Nuevas'}: {stats.pending}</div>
            <div className="text-yellow-500">{targetLanguage === 'eu' ? 'Ikasten' : 'Estudiadas'}: {stats.studied}</div>
            <div className="text-green-500">{targetLanguage === 'eu' ? 'Ikasiak' : 'Aprendidas'}: {stats.learned}</div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
          <div className="bg-white/5 p-8 rounded-3xl border border-white/10">
            <h3 className="text-xl font-bold mb-6 text-white flex items-center gap-2">
              ...
              {targetLanguage === 'eu' ? 'Kategoriaren Arabera' : 'Por Categoría'}
            </h3>
